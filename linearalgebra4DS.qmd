...
We are dealing with vectors, vector basis, .rouge{enter the world of Linear Algebra.}
...
1 & 1 & -1\cr
1 & 0 & 1
\end{pmatrix}, \quad rank(X_2) = 3;\quad \quad X_3=\begin{pmatrix}
1 & 1 & -1 \cr
1 & 1 & -1\cr
1 & 0 & 1
...
p1 <- ggplot() + 
  ...
  annotate( "text", label = c("x", "Dx"), x=c(-0.1, -0.2), y=c(0.5, 0.1), col = couleur[2], size = 10) +
  xlab('') + ylab('')
