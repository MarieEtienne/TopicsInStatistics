...
As the variables are scaled, $ \frac{1}{n} \sum_{i=1}^n  (x_{ik} - x_{.,k})^2 =  s^2_k = 1.$ and $I_G=d.$


[Total inertia with scaled centred variables is $d$]{.rouge}
...
\begin{align}
I & =  \frac{1}{n} \sum_{i}^n d^2(X_{i,.}, 0) =  \frac{1}{n} \sum_{i}^n \norm{X_{i,.}}\\
& =  \frac{1}{n} \sum_{i}^n x_i^\intercal x_i \cr
& = tr( \frac{1}{n} \sum_{i}^n x_i^\intercal x_i) \cr
& = tr( \frac{1}{n} \sum_{i}^n x_i x_i^\intercal ) \cr
& = tr( \frac{1}{n} X^\intercal  X) \cr
\end{align}
...
-   Considering the SVD of $X$, we have $X= P^\intercal D Q$ and so 
       $$\frac{1}{n} X^\top  X =  X^\top W X = Q^\intercal D^2 Q.$$
